...
void marcarTareaComoCompletada(Map *mapa) {
  ...
  if (tarea == NULL) {
    ...
    if (strlen(tarea->tareasPrecedentes) > 0) {
      printf("Advertencia: Esta tarea tiene precedentes. ¿Estás seguro de que "
             "deseas marcarla como completada? (s/n): ");
      char respuesta;
      scanf(" %c", &respuesta);
    ...
    printf("Tarea marcada como completada y eliminada de la lista de tareas "
           "por hacer.\n");
  }
}
